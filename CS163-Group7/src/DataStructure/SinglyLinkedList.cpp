...
void SinglyLinkedList::initUI() {
    ...
    buttons.push_back(Button(sf::Vector2f(btnW, btnH), sf::Vector2f(colX, startY), "Init Random", font, [this]() {
        std::vector<int> r;
        for (int i = 0; i < 5; ++i) r.push_back(rand() % 100);
        init(r);
    }));
...
void SinglyLinkedList::beginInsertSteps(int value, int idx) {
    ...
    if (!isStepByStep) {
        ...
        VisualStep s;
        s.message = "Inserted " + std::to_string(value) + " at index " + std::to_string(idx) + " (Run at Once)";
...
void SinglyLinkedList::beginDeleteSteps(int idx) {
    ...
    if (!isStepByStep) {
        ...
        VisualStep s;
        s.message = "Deleted node at index " + std::to_string(idx) + " with value " + std::to_string(value) + " (Run at Once)";
...
void SinglyLinkedList::beginUpdateSteps(int idx, int newValue) {
    if (idx < 0 || idx >= (int)nodes.size())
        return;
    ...
    std::vector<std::string> code = {
        "void updateNode(int idx, int newValue) {",
        "    Node* curr = head;",
        "    for (int i = 0; i < idx; ++i)",
        "        curr = curr->next;",
        "    curr->value = newValue;",
