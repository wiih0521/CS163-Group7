...
void AVLTree::initUI() {
    ...
    float startX = winW - codeBoxW + 30.0f - (maxCols * btnW + (maxCols - 1) * gap);
    float colX = startX;
    ...
    buttons.push_back(Button(sf::Vector2f(btnW, btnH), sf::Vector2f(colX, startY), "Insert", font, [this, colX, btnW, startY]() {
        activeDialog = std::make_unique<InputDialog>("Insert Node", std::vector<InputDialog::Field>{
            {"Value:", "Val"}
        }, font, [this](const std::vector<std::string>& results) {
            if (!results[0].empty()) {
    ...
    buttons.push_back(Button(sf::Vector2f(btnW, btnH), sf::Vector2f(colX, startY), "Update Node", font, [this, colX, btnW, startY]() {
        activeDialog = std::make_unique<InputDialog>("Update Node", std::vector<InputDialog::Field>{
            {"Old Value:", "Old"},
            {"New Value:", "New"}
        }, font, [this](const std::vector<std::string>& results) {
            if (!results[0].empty() && !results[1].empty()) {
                try { beginUpdateSteps(std::stoi(results[0]), std::stoi(results[1])); } catch(...) {}
            }
            activeDialog.reset();
        }, [this]() { activeDialog.reset(); }, winW, winH, sf::Vector2f(colX + btnW + 15.f, startY));
    }));
    ...
    buttons.push_back(Button(sf::Vector2f(btnW, btnH), sf::Vector2f(colX, startY), "Delete", font, [this, colX, btnW, startY]() {
        activeDialog = std::make_unique<InputDialog>("Delete Node", std::vector<InputDialog::Field>{
            {"Value to delete:", "Val"}
        }, font, [this](const std::vector<std::string>& results) {
            if (!results[0].empty()) {
                try { beginDeleteSteps(std::stoi(results[0])); } catch(...) {}
            }
            activeDialog.reset();
        }, [this]() { activeDialog.reset(); }, winW, winH, sf::Vector2f(colX + btnW + 15.f, startY));
    }));
...
void AVLTree::draw(sf::RenderWindow& window) {
    ...
    title.setCharacterSize(24); title.setFillColor(sf::Color::White);
    sf::FloatRect tb = title.getLocalBounds(); title.setOrigin(tb.left + tb.width/2.f, 0.f); title.setPosition(centerX, 10); window.draw(title);
...
void AVLTree::removeNodeSim(SimNode** nodeRef, int value, std::vector<VisualStep>& steps, SimNode** rootRef, int* valToReplace) {
    ...
    takeSnapshot("Delete " + std::to_string(value) + ": looking at " + std::to_string(node->value), {node->value}, sf::Color(220, 180, 0), -1, -1, {rmLine, rmLine+1});
...
void AVLTree::beginUpdateSteps(int oldVal, int newVal) {
    if (!animSteps.empty()) {
        if (commitOp) {
            if (animSteps.front().treeSnapshot) applySimStructure(animSteps.front().treeSnapshot);
            commitOp();
    ...
    beginDeleteSteps(oldVal);
    auto deleteSteps = animSteps;
    auto deleteCommit = commitOp;

    VisualStep sep;
    sep.message = "Deleted " + std::to_string(oldVal) + ". Now inserting " + std::to_string(newVal) + "...";
    sep.highlightColor = sf::Color(100, 180, 255);
    if (!deleteSteps.empty() && deleteSteps.back().treeSnapshot)
        sep.treeSnapshot = copySimTree(deleteSteps.back().treeSnapshot);
    deleteSteps.push_back(sep);

    commitOp = nullptr; 
    animSteps.clear();

    std::map<int, TreeNode*> backupCache = nodeCache;
    if (deleteCommit) deleteCommit();

    std::vector<TreeNode*> safeGc = gcNodes; 
    gcNodes.clear();

    beginInsertSteps(newVal);
    auto insertSteps = animSteps;
    auto insertCommit = commitOp;

    animSteps = deleteSteps;
    for (auto& s : insertSteps)
        animSteps.push_back(s);

    commitOp = insertCommit;
    nodeCache = backupCache;
    gcNodes = safeGc; 
