...
void GraphMST::initUI() {
    ...
    buttons.push_back(Button(sf::Vector2f(btnW, btnH), sf::Vector2f(colX, startY), "Add Node", font, [this, colX, btnW, startY]() {
        activeDialog = std::make_unique<InputDialog>("Add Node", std::vector<InputDialog::Field>{
            {"Node ID:", "ID"}
        }, font, [this](const std::vector<std::string>& results) {
            if (!results[0].empty()) addNode(std::stoi(results[0]));
            activeDialog.reset();
    ...
    buttons.push_back(Button(sf::Vector2f(btnW, btnH), sf::Vector2f(colX, startY), "Add Edge", font, [this, colX, btnW, startY]() {
        activeDialog = std::make_unique<InputDialog>("Add Edge", std::vector<InputDialog::Field>{
            {"From Node:", "From"},
            {"To Node:", "To"},
            {"Weight:", "Wt"}
        }, font, [this](const std::vector<std::string>& results) {
