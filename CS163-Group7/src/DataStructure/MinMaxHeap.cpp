...
MinMaxHeap::MinMaxHeap(float windowWidth, float windowHeight) : isMinHeap(true) {
    winW = windowWidth;
    winH = windowHeight;
    // Load the font used to draw numbers on the nodes and UI messages
...
void MinMaxHeap::initUI() {
    ...
    // Position UI elements at the bottom of the screen
    ...
    buttons.push_back(Button(sf::Vector2f(120,30), sf::Vector2f(560, uiY2), "Insert", font, [this]() {
        std::string v = textInputs[1].getText(); 
        if (!v.empty()) beginInsertSteps(std::stoi(v));
    }));
    ...
    buttons.push_back(Button(sf::Vector2f(90,30), sf::Vector2f(940, uiY2), "Search", font, [this]() {
        std::string v = textInputs[2].getText(); 
...
void MinMaxHeap::beginInsertSteps(int value) {
    ...
    // If the user opts to run instantly without step-by-step effects
    if (!isStepByStep) {
        insert(value);
        VisualStep s; 
    ...
    //Step 1: Push the new node to the actual data array
    ...
    nodes.push_back(n);
    recalculateTargetPositions(); // Calculate where it needs to fly to
    int idx = (int)rawData.size() - 1;

    // Scripting: Highlight the newly added node at the end of the array
    ...
    // Step 2: Simulation. Use a virtual array 'sim' to run HeapifyUp.
    // We record the comparison/swap steps into animSteps, but do not alter the real data yet.
    std::vector<int> sim = rawData;
    int si = idx;
    while (si > 0) {
        int parent = (si - 1) / 2;
        // Check if a swap with the parent node is needed
    ...
    // Step 3: Save the commit function
    int insertedIdx = idx;
...
void MinMaxHeap::beginSearchSteps(int value) {
    ...
    // Traversing the tree using stack
    std::vector<int> stack;
    if (!rawData.empty()) {
        if (canContain(rawData[0])) {
            stack.push_back(0);// If the root's value satisfies, push it to stack
        } else {
            // Stop immediately if the condition isn't met
            VisualStep s; 
            s.highlighted = {0}; 
    ...
    while (!stack.empty()) {
        ...
        VisualStep s; 
        s.highlighted = {idx};
        if (rawData[idx] == value) {// Found the node
        ...
        // Check and push child nodes into stack if they met the condition
    ...
    // Record the final result into animStep
    VisualStep done;
    if (found) {
