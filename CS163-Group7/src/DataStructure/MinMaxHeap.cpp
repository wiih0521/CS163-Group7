...
void MinMaxHeap::beginDeleteAtSteps(int idx) {
    ...
    if (!isStepByStep) {
        rawData[idx] = rawData.back(); rawData.pop_back();
        nodes[idx] = nodes.back(); nodes.pop_back();
        if (idx < (int)rawData.size()) { heapifyDown(idx); heapifyUp(idx); }
        recalculateTargetPositions();
        VisualStep s;
...
void MinMaxHeap::beginUpdateSteps(int idx, int newValue) {
    if (idx < 0 || idx >= (int)rawData.size())
        return;
