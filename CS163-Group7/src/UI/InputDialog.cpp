...
InputDialog::InputDialog(
  ...
  backgroundOverlay.setFillColor(
      Theme::withAlpha(sf::Color::Black, position.x >= 0.f ? 0 : 150));
  ...
  for (const auto &field : fields) {
    ...
    TextInput input(sf::Vector2f(180.f, 30.f),
                    sf::Vector2f(boxX + 150.f, currentY), font,
                    field.placeholder);
  ...
  buttons.push_back(Button(sf::Vector2f(100.f, 35.f),
                           sf::Vector2f(boxX + 60.f, currentY + 10.f), "OK",
                           font, [this]() {
  ...
  buttons.push_back(Button(sf::Vector2f(100.f, 35.f),
                           sf::Vector2f(boxX + 190.f, currentY + 10.f),
                           "Cancel", font, [this]() { cancelCallback(); }));
}
...
bool InputDialog::handleEvent(const sf::Event &event,
  ...
  // Block events from passing through to underlying elements
